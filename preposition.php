<style>
    ...
    .newStyle {
        ...
        background-image: linear-gradient(315deg, #accf52 0%, #f5f7fa 74%); */
...
if (!isset($_POST['submitPreposition'])) {
    $preposition = $_POST['preposition'];
    echo "<script>console.log(' working ')</script>";
    $username = $_SESSION['userUid'];
    if (empty($preposition)) {
        header("refresh, url = http://localhost/my%20project/main.php");
        exit();
    } else {
        $sql = "INSERT INTO preposition(uidUsers, text) VALUES ('$username', '$preposition')";

        $result = mysqli_query($conn, $sql);
        $sql = "SELECT * FROM preposition ORDER BY idPrep desc";
        $result = mysqli_query($conn, $sql);
        $votecount = 0;
        ...
        echo "<script>document.getElementById('prepositionsArea').value = '';</script>";
    }
} else {
    echo "<script>alert('button isset eroor!')</script>";
}
